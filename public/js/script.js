...
async function loadEventsFromServer() {
  try {
    ...
    upcomingEvents = allEvents.map(event => {
      // Ensure tags/categories is always an array
      let categories = [];
      
      // Try to parse tags if it's a string (JSON)
      if (event.tags) {
      ...
      if (categories.length === 0 && event.categories) {
      ...
      // Default to weekdays if no tags/categories found
...
function initializeBooking() {
  ...
  // Note: Booking modal only exists on index.html, not on events.html
  if (!bookingModal || !bookingForm) {
    // Only warn if we're on a page that should have the booking modal
    if (document.querySelector('[data-upcoming-list]')) {
      console.warn('Booking modal elements not found. Booking functionality may not work.');
    }
    return;
  }
...
function initializeQuickCards() {
  ...
  quickCards.forEach((card, index) => {
    ...
    newCard.addEventListener('click', (e) => {
      ...
      // Map card text to category
      let category = '';
      if (cardText.includes('Music')) {
        category = 'Music';
      } else if (cardText.includes('Conference')) {
        category = 'Conference';
      } else if (cardText.includes('Celebration')) {
        category = 'Celebration';
      } else if (cardText.includes('Games') || cardText.includes('Sports')) {
        category = 'Sports';
      }
      ...
      if (category) {
        ...
        if (categorySelect) {
          ...
          if (typeof window.updateCategoryFilter === 'function') {
            window.updateCategoryFilter(category);
          }
          // Perform search
          if (typeof window.performHomepageSearch === 'function') {
            window.performHomepageSearch();
          }
          // Scroll to events section
          setTimeout(() => {
